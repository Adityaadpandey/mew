...
export function DocumentEditorV2() {
    ...
    const loadDocument = async () => {
      ...
      try {
        ...
        console.log('✅ Document loaded:', {
          id: document.id,
          blocks: document.content?.blocks?.length || 0,
        })
      } catch (error) {
  ...
  const saveDocument = useCallback(async () => {
    ...
    console.log('💾 Saving document:', {
      ...
      blocks: currentDocument.content?.blocks?.length || 0,
    })
  ...
  // Autosave hook
  useAutosaveV2(
    currentDocument?.content, // Content to watch
    saveDocument, // Save function
  ...
  if (!currentDocument) {
    return (
      <div className="flex items-center justify-center h-full">
        <p className="text-sm text-neutral-500">No document selected</p>
      </div>
    )
  }
